...
export function CategoriasTab() {
  ...
  async function handleSalvar(valores: CategoriaFormValues) {
    if (categoriaEmEdicao) {
      await atualizarCategoria(categoriaEmEdicao.id, valores);
    } else {
  ...
  async function handleExcluir(categoria: Categoria) {
    const confirmado = window.confirm(
      `Excluir a categoria "${categoria.nome}"? Essa ação não pode ser desfeita.`
    );
  ...
  async function handleMover(categoria: Categoria, direcao: "up" | "down") {
    await moverCategoria(categoria.id, direcao);
    await carregar();
  }
